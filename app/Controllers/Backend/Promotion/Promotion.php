		...
		if ($this->request->getMethod() == 'post') {
			...
			if ($this->validate($validate['validate'], $validate['errorValidate'])) {
				...
				if ($resultid > 0) {
					...
					write_audit_log($user['id'], 'Thêm Mới', 'Thêm Mới Khuyến Mãi Thành Công');
		...
		if ($this->request->getMethod() == 'post') {
			...
			if ($this->validate($validate['validate'], $validate['errorValidate'])) {
				...
				if ($flag > 0) {
					...
					write_audit_log($user['id'], 'Cập Nhật', 'Cập Nhật Khuyến Mãi Thành Công');
					return redirect()->to(BASE_URL . 'backend/promotion/promotion/index');
				}
			} else {
				$this->data['validate'] = $this->validator->listErrors();
	...
	public function delete($id = 0)
	{
		...
		if ($this->request->getPost('delete')) {
			...
			if ($flag > 0) {
				...
				write_audit_log($user['id'], 'Xóa', 'Xóa Khuyến Mãi Thành Công');
			} else {
				$session->setFlashdata('message-danger', 'Có vấn đề xảy ra, vui lòng thử lại!');
			}
			return redirect()->to(BASE_URL . 'backend/promotion/promotion/index');
