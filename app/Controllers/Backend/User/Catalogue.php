	...
	public function index($page = 1)
	{
		...
		if ($config['total_rows'] > 0) {
			...
			$this->data['userCatalogueList'] = $this->AutoloadModel->_get_where([
				...
				(SELECT COUNT(id) FROM user WHERE user.catalogueid = user_catalogue.id AND user.isadmin = 0) as total_user,
				(SELECT COUNT(id) FROM user WHERE user.catalogueid = user_catalogue.id AND user.publish = 0 AND user.isadmin = 0) as de_active,
				(SELECT COUNT(id) FROM user WHERE user.catalogueid = user_catalogue.id AND user.publish = 1 AND user.isadmin = 0) as active',
				'table' => $this->data['module'],
				'where' => $where,
				'keyword' => $keyword,
